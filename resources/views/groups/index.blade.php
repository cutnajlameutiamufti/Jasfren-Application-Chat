    ...
    <div class="py-6 h-[calc(100vh-64px)] bg-gray-50">
        <div class="max-w-7xl mx-auto h-full sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm rounded-2xl h-full border border-gray-100 flex">
                ...
                <div class="w-full md:w-80 lg:w-96 border-r border-gray-100 flex flex-col h-full bg-white font-sans">
                    ...
                    <div class="flex-1 overflow-y-auto divide-y divide-gray-50">
                        @forelse($groups as $group)
                            ...
                            @endphp
                            <a href="{{ route('groups.show', $group->id) }}" class="flex items-center px-4 py-3.5 hover:bg-blue-50/30 transition-all duration-200 group">
                                <!-- Avatar -->
                                ...
                                <div class="ml-3 flex-1 min-w-0">
                                    ...
                                    <div class="flex justify-between items-center mt-1">
                                        <p class="text-xs text-gray-500 truncate pr-2">
                                            @if($lastMsg)
                                                <span class="text-gray-600 font-semibold">{{ $lastMsg->sender->id === auth()->id() ? 'Anda' : $lastMsg->sender->name }}:</span>
                                                {{ $lastMsg->body }}
                                            @else
                                    ...
                                    <p class="text-[10px] text-gray-400 mt-1">{{ $memberCount }} Anggota</p>
                                </div>
                            </a>
                        @empty
                            <div class="flex flex-col items-center justify-center h-full text-center px-6 py-12">
                                ...
                                <p class="text-xs text-gray-500 mt-1 max-w-[200px]">Buat grup baru dan undang teman untuk mulai berdiskusi bersama.</p>
                                <a href="{{ route('groups.create') }}" class="mt-4 px-4 py-2 bg-blue-600 text-white text-xs font-semibold rounded-xl hover:bg-blue-700 shadow-sm transition-all duration-200">
                                    Buat Grup Baru
                                </a>
                            </div>
                        @endforelse
                ...
                <!-- Kanan: Detail Grup Default (Empty State) -->
                <div class="hidden md:flex flex-1 flex-col items-center justify-center bg-gray-50/50 p-8 text-center h-full">
                    <div class="max-w-md flex flex-col items-center">
                        ...
                        <h2 class="text-xl font-bold text-gray-900">Jasfren Grup</h2>
                        <p class="text-sm text-gray-500 mt-2 max-w-sm">Pilih grup dari daftar obrolan di sebelah kiri untuk mulai berdiskusi seru bersama banyak orang secara real-time.</p>
                        <div class="mt-8 flex items-center space-x-2 text-xs text-gray-400">
                            ...
                            <span>Diskusi aman dan terorganisir</span>
                        </div>
                    </div>
                </div>
